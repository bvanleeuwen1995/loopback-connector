...
SQLConnector.prototype.buildSelect = function(model, filter, options) {
  ...
  var selectStmt = new ParameterizedSQL('SELECT ' +
    ...
    // Check if we have the distinct filter
    (filter.distinct ?
      'DISTINCT ON('+this.columnEscaped(model, filter.distinct)+') '+this.tableEscaped(model)+'.* ' : // Add the distinct
      this.buildColumnNames(model, filter)) + // Add the normal select
    ' FROM ' + this.tableEscaped(model)
  );

  if (filter.innerJoin) {
    var targetModel = this.getModelDefinition(filter.innerJoin.targetTable);
    var sourceTable = this.tableEscaped(model);
    var targetTable = this.tableEscaped(filter.innerJoin.targetTable);
    var sourceColumn = this.columnEscaped(targetModel, filter.innerJoin.sourceColumn);
    selectStmt.sql += ' INNER JOIN '+targetTable+' ON '+sourceTable+'.'+sourceColumn+' = '+targetTable+'."id"'
  }
  ...
  if (filter) {
    ...
    if (filter.order) {
      ...
      if (filter.distinct) {
        // Add the filter column as the filter column
        orderBy += this.columnEscaped(model, filter.distinct) +' ASC, ';
      }
      orderBy += this.buildOrderBy(model, filter.order);
      selectStmt.merge(orderBy);
    }
